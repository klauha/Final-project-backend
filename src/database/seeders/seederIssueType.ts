...
export const issueTypeSeedDatabase = async () => {
    try {
      ...
      await IssueType.save({
        name: "Errores en el sitio web",
        description: "problemas de carga, errores de página, problemas de navegación.",
        department: {id: 1},
      });

      await IssueType.save({
        name: "Problemas de integración",
        description: "errores en la pasarela de pago, problemas con API de terceros.",
        department: {id: 1},
      });

      await IssueType.save({
        name: "Errores de registro o inicio de sesión",
        description: "problemas para crear una cuenta de usuario nueva, errores al iniciar sesión en una cuenta existente.",
        department: {id: 1},
      });

      await IssueType.save({
        name: "Problemas con promociones",
        description: "cupones que no funcionan, descuentos no aplicados correctamente.",
        department: {id: 2},
      });

      await IssueType.save({
        name: "Problemas de seguimiento de ventas",
        description: "errores en el registro de ventas, discrepancias en los datos de ventas.",
        department: {id: 2},
      });

      await IssueType.save({
        name: "Devoluciones y reembolsos",
        description: "problemas con procesos de devolución, reembolsos no procesados.",
        department: {id: 3},
      });

      await IssueType.save({
        name: "Consultas generales de clientes",
        description: "preguntas sobre productos, políticas de envío.",
        department: {id: 3},
      });

      await IssueType.save({
        name: "Quejas de clientes sobre la experiencia de compra.",
        description: "problemas con el producto, problemas con la atención recibida.",
        department: {id: 3},
      });

      await IssueType.save({
        name: "Retrasos en la entrega",
        description: "paquetes perdidos, retrasos en el envío.",
        department: {id: 4},
      });

      await IssueType.save({
        name: "Problemas con la logística de inventario",
        description: "discrepancias de stock, productos dañados durante el envío.",
        department: {id: 4},
      });

      await IssueType.save({
        name: "Consultas de seguimiento de envíos",
        description: "clientes que buscan información sobre el estado de su pedido.",
        department: {id: 4},
      });

      await IssueType.save({
        name: "Problemas con pagos",
        description: "transacciones fallidas, pagos duplicados.",
        department: {id: 5},
      });

      await IssueType.save({
        name: "Consultas sobre facturas y pagos",
        description: "clientes que necesitan asistencia con sus facturas, disputas de cargos.",
        department: {id: 5},
      });

      await IssueType.save({
        name: "Problemas con la contabilidad",
        description: "errores en registros financieros, discrepancias en balances.",
        department: {id: 5},
      });        
